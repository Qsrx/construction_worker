 ...
 */
public class ClassSearchRvAdapter extends BaseQuickAdapter<ClassSearchBean.DataBean,BaseViewHolder>{

    public ClassSearchRvAdapter(int layoutResId) {
        super(R.layout.system_class_rv_layout);
    }

    @Override
    protected void convert(BaseViewHolder helper,ClassSearchBean.DataBean item) {
        ShapedImageView siv = helper.itemView.findViewById(R.id.system_class_rv_siv);
        Glide.with(mContext).load(item.getTaocan_pic()).into(siv);
